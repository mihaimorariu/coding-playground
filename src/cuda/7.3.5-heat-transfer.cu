...
int main(void) {
  ...
  HANDLE_ERROR(cudaBindTexture2D(NULL, tex_const_src, data.dev_const_src,
                               desc, DIM, DIM, sizeof(float) * DIM));
  HANDLE_ERROR(cudaBindTexture2D(NULL, tex_in, data.dev_in_src,
                               desc, DIM, DIM, sizeof(float) * DIM));
  HANDLE_ERROR(cudaBindTexture2D(NULL, tex_out, data.dev_out_src,
                               desc, DIM, DIM, sizeof(float) * DIM));
